...
class Evaluator(object):
    def __init__(self, dataset, class_num, norm_mean, norm_std, network, multi_scales, 
                ...
                batch_size=1, use_amp=False, amp_dtype=torch.bfloat16):
        self.eval_time = 0
    ...
    def whole_eval(self, img, output_size, device=None):
        ...
        for s in self.multi_scales:
            ...
            processed_pred += cv2.resize(tensor_to_numpy_fp32(pred),
    ...
    def scale_process(self, img, ori_shape, crop_size, stride_rate,
        ...
        data_output = cv2.resize(tensor_to_numpy_fp32(score),
    ...
    def scale_process_rgbX_batch(self, imgs, modal_xs, ori_shape, crop_size, stride_rate, device=None):
        ...
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            ...
            for img, modal_x in zip(imgs, modal_xs):
                img_pad, margin = pad_image_to_shape(img, crop_size, cv2.BORDER_CONSTANT, value=0)
                modal_x_pad, _ = pad_image_to_shape(modal_x, crop_size, cv2.BORDER_CONSTANT, value=0)
                img_pads.append(img_pad)
                modal_x_pads.append(modal_x_pad)
            ...
            pad_rows = img_pads[0].shape[0]
            pad_cols = img_pads[0].shape[1]
            r_grid = int(np.ceil((pad_rows - crop_size[0]) / stride[0])) + 1
            c_grid = int(np.ceil((pad_cols - crop_size[1]) / stride[1])) + 1
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    batch_pairs = []
                    for batch_idx in range(batch_size):
                        if len(modal_x_pads[batch_idx].shape) == 2:
                            modal_x_sub = modal_x_pads[batch_idx][s_y:e_y, s_x:e_x]
                        else:
                            modal_x_sub = modal_x_pads[batch_idx][s_y:e_y, s_x:e_x, :]
                        batch_pairs.append(
                            (
                                img_pads[batch_idx][s_y:e_y, s_x:e_x, :],
                                modal_x_sub,
                            )
                        )
