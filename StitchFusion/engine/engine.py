...
from utils.pyt_utils import ensure_dir, extant_file, link_file, load_model, parse_devices
...
class Engine(object):
    ...
    def inject_default_parser(self):
        ...
        p.add_argument('-p', '--port', type=str, default='16005', dest='port', help='port for init_process_group')
    ...
    def save_checkpoint(self, path):
        ...
        for k, v in self.state.model.state_dict().items():
            key = k[7:] if k.split('.')[0] == 'module' else k
    ...
    def save_and_link_checkpoint(self, checkpoint_dir, log_dir, log_dir_link):
        ...
        current_epoch_checkpoint = osp.join(checkpoint_dir, 'stitchfusion_epoch_{}.pth'.format(self.state.epoch))
