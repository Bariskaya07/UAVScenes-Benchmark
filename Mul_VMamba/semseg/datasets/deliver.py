...
class DELIVER(Dataset):
    ...
    def __init__(self, root: str = 'data/DELIVER', split: str = 'train', transform = None, modals = ['img'], case = None, aux_channels: int = 3) -> None:
        super().__init__()
        ...
        self.modals = modals
        self.split = split
        self.aux_channels = aux_channels  # 2 for native LiDAR (Range+Intensity), 3 for backward compat
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        if 'lidar' in self.modals:
            sample['lidar'] = self._open_img(x2, is_lidar=True)
    ...
    def _open_img(self, file, is_lidar=False):
        img = io.read_image(file)
        C, H, W = img.shape
        if C == 4:
            img = img[:3, ...]
        if C == 1:
            # For LiDAR, use aux_channels setting
            if is_lidar and self.aux_channels == 2:
                # For 2-channel mode, don't stack - will be handled separately
                # This path is for when we have Range+Intensity as separate files
                img = img.repeat(2, 1, 1)  # Placeholder: will load both channels
            else:
