...
class RandomColorJitter:
    ...
    - hue: 0.1 (range: -0.1 to 0.1)
    """
    def __init__(self, p=0.5, brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1) -> None:
        self.p = p
    ...
    def __call__(self, sample: list) -> list:
        ...
        if random.random() < 0.5:
            ...
            sample['img'] = TF.adjust_brightness(sample['img'], brightness_factor)

        if random.random() < 0.5:
            contrast_factor = random.uniform(1 - self.contrast, 1 + self.contrast)
            sample['img'] = TF.adjust_contrast(sample['img'], contrast_factor)

        if random.random() < 0.5:
            saturation_factor = random.uniform(1 - self.saturation, 1 + self.saturation)
            sample['img'] = TF.adjust_saturation(sample['img'], saturation_factor)

        if random.random() < 0.5:
            hue_factor = random.uniform(-self.hue, self.hue)
            sample['img'] = TF.adjust_hue(sample['img'], hue_factor)
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    """
    Training augmentation pipeline (standardized with CMNeXt for fair comparison).
