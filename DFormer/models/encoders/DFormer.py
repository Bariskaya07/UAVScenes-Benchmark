...
class DFormer(BaseModule):
    def __init__(
        ...
        drop_path_rate=0.1,
        init_cfg=None,
        aux_in_chans=1,  # Configurable: 1 for UAVScenes HAG, 2 for DELIVER LiDAR
    ):
        super().__init__()
        ...
        # aux_in_chans: 1 for UAVScenes HAG, 2 for DELIVER LiDAR
        stem_e = nn.Sequential(
            nn.Conv2d(aux_in_chans, dims[0] // 4, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(dims[0] // 4),
            nn.GELU(),
            nn.Conv2d(dims[0] // 4, dims[0] // 2, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(dims[0] // 2),
        )
        print(f"[DFormer] Auxiliary encoder input channels: {aux_in_chans}")
