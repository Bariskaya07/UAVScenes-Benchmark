"""
DELIVER Dataset for TokenFusion Training
...
# Import augmentations if available
try:
    from .augment import get_train_transform, get_val_transform
except ImportError:
    get_train_transform = None
...
class DELIVERDataset(Dataset):
    ...
    Compatible with TokenFusion's data loading interface.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            transform: Transform function (from augment.py)
            modals: List of modalities to use
            case: Weather/condition filter
            aux_channels: Number of channels for LiDAR (2 for native, 3 for compat)
        """
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        # Build sample dict (TokenFusion format)
        sample = {
            'rgb': rgb,
            'lidar': lidar,  # Using 'lidar' instead of 'hag' for DELIVER
            'label': label,
        }
...
def get_deliver_dataset(cfg, split='train'):
    """
    Factory function to create DELIVER dataset with proper transforms.

    Args:
        cfg: Config object with dataset settings
        split: 'train', 'val', or 'test'

    Returns:
        DELIVERDataset instance
    """
    root = getattr(cfg, 'data_root', 'data/DELIVER')
    aux_channels = getattr(cfg, 'aux_channels', 3)
    modals = getattr(cfg, 'modals', ['rgb', 'lidar'])

    if split == 'train' and get_train_transform is not None:
        transform = get_train_transform(cfg)
    elif get_val_transform is not None:
