"""
UAVScenes Dataset for TokenFusion

RGB + HAG (Height Above Ground) multi-modal semantic segmentation dataset.
Compatible with fair comparison settings from CMNeXt and DFormerv2.
...
class UAVScenesDataset(Dataset):
    """
    UAVScenes RGB + HAG Dataset for TokenFusion.
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            'hag': hag,
            'label': label,
            'inputs': ['rgb', 'hag']  # For transform compatibility
        }
        ...
        # Remove inputs key after transform
        if 'inputs' in sample:
            del sample['inputs']
...
if __name__ == '__main__':
    ...
    print(f"HAG shape: {sample['hag'].shape}")
    print(f"Label shape: {sample['label'].shape}")
    print(f"Label unique values: {np.unique(sample['label'])}")
