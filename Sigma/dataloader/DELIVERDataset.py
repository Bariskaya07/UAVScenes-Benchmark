"""
DELIVER Dataset for Sigma (Siamese Mamba Network) Training
...
class DELIVERDataset(data.Dataset):
    """
    DELIVER dataset for Sigma training.
    ...
    def __getitem__(self, index):
        ...
        # Convert LiDAR to required channels
        if self.aux_channels == 1:
            x = lidar[..., np.newaxis] if len(lidar.shape) == 2 else lidar[:, :, :1]
        elif self.aux_channels == 2:
            if len(lidar.shape) == 2:
                x = np.stack([lidar, lidar], axis=-1)
            else:
                x = lidar[:, :, :2] if lidar.shape[2] >= 2 else np.stack([lidar[:,:,0]]*2, axis=-1)
        else:
            if len(lidar.shape) == 2:
                x = cv2.merge([lidar, lidar, lidar])
            else:
                x = lidar[:, :, :3] if lidar.shape[2] >= 3 else np.stack([lidar[:,:,0]]*3, axis=-1)
    ...
    def _load_lidar(self, filepath):
        ...
        # Convert to uint8 range for Sigma preprocessing compatibility
